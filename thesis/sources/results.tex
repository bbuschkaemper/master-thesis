...
However, these synthetic dataset variants are not representative of real-world datasets,
since the underlying images between images are the same (from the same real-world dataset).
Therefore, we will redo the relationship selection method evaluation on real-world datasets
to conclude how they perform in different environments and (hopefully) concur on a single parameter configuration
that works well across all scenarios.

\subsubsection{Real-World Datasets}

% TODO Evaluation WordNet => Caltech100 - Caltech256
% TODO WordNet cite other papers for references
% TODO MNIST - SVHN as verification that it works well (not applyable by itself)
% TODO Close-up examples

\subsection{Universal Taxonomy Generation}

% TODO Manual look at caltech101-caltech256 taxonomy
% TODO Also explaind and plot training
% TODO Explain what differences we make to Bevandic taxonomy generation

% TODO OBSERVED: Low naive threshold doesn't work as well for real-world as for synthetic
% TODO Probably because synthetic were trained on same dataset and not good enough for comparison
% TODO Can we find dataset with real-world mapping??? (Maybe based on labels?)
