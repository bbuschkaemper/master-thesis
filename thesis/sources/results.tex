...
To create these domain-shifted variants, we apply two distinct sets of transformations to the original Caltech-256 images.
...
\begin{figure}[ht]
      \centering
      \scalebox{0.35}{\input{figures/caltech256_2domain_variant_domain_shifted.pgf}}

      \caption{Accuracy curves for domain A and B of the domain-shifted synthetic dataset variants
            on the Caltech256 dataset.}
      \label{fig:training_domain_shifted_variants}
\end{figure}

\begin{figure}[ht]
      \centering
      \scalebox{0.35}{\input{figures/naive_threshold_method_precision_recall_domain_shifted.pgf}}
      \caption{Precision-recall curves for the naive threshold method on the domain-shifted synthetic dataset variants.}
      \label{fig:naive_threshold_method_precision_recall_domain_shifted}
\end{figure}

\begin{figure}[ht]
      \centering
      \scalebox{0.35}{\input{figures/density_threshold_method_precision_recall_domain_shifted.pgf}}

      \caption{Precision-recall curves for the density threshold method on the domain-shifted synthetic dataset variants.}
      \label{fig:density_threshold_method_precision_recall_domain_shifted}
\end{figure}

\begin{figure}[ht]
      \centering
      \scalebox{0.35}{\input{figures/hypothesis_method_precision_recall_domain_shifted.pgf}}

      \caption{Precision-recall curves for the relationship hypothesis method on the domain-shifted synthetic dataset variants.}
      \label{fig:hypothesis_method_precision_recall_domain_shifted}
\end{figure}

\input{figures/evaluation_results_domain_shifted.tex}
\input{figures/relationship_methods_domain_shifted_results.tex}

We train the domain-shifted models with the same architecture and training configuration
as the original models. The evaluation results can be seen in Table~\ref{tab:evaluation_results_domain_shifted}.
We observe an average accuracy of approx. $0.72$, around $8\%$ lower than the original models.

The final method selection evaluation results are shown in Table~\ref{tab:relationship_methods_domain_shifted_results}.

% TODO Train all 4 variants and compare the final results
% TODO Observation: We can observe large cluster buildings like with the real taxonomies (did this also happen in the normal synthetic variants?)
...
% TODO Make algorithm more efficient: Calculate mapping matrices at the beginning and use these
% TODO Include domainid in batch so we can alternate between domains during training
