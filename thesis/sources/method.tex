...
\subsection{Taxonomy Difference Metrics}

We created our synthetic datasets to have a known ground truth of relationships between classes.
Now, we need to find metrics to compare the predicted relationships from our taxonomy generation methods
with the ground truth calculated from the synthetic datasets.

\subsubsection{Universal Class Graph}

For our metrics, we want the graphs to have the same nodes (while the edges can differ).
Every universal class in our taxonomy has either one or two incoming edges
from domain classes, which means that we can represent the universal classes as edges in a new graph
(with single incoming edges being represented as a self-loop).
...
\subsubsection{Edge Difference Ratio}
...
normalised by the size of the union of the two graphs' edges (i.e. all edges without duplicates).

We define the EDR as follows, using adjacency matrices $A_1$ and $A_2$ for the two universal class graphs:

\begin{equation}
    \text{EDR}(G_1, G_2) = \frac{\sum_{i,j} |A_1(i,j) - A_2(i,j)|}{\sum_{i,j} \max(A_1(i,j), A_2(i,j))}
\end{equation}

This definition works for both weighted and unweighted graphs, making it suitable for our use case.
The values of the EDR range from 0 to 1, where 0 indicates identical graphs and 1 indicates completely different graphs
(without a single edge in common).